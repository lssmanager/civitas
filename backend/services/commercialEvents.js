...
async function applyCommercialEvent(normalized, row) {
  ...
  await db.update(commercialEvents).set({ status: EVENT_STATUSES.APPLIED, organizationProfileId: profile.id, logtoOrganizationId: profile.logtoOrganizationId, seatDelta: next.seatTotal - profile.seatTotal, commercialStatusAfter: next.settings.commercial.status, logtoChangeSummary: { customData: "civitasCommercial", accessEnforcementMode: "pending_access_enforcement_no_logto_role_or_membership_mutation", untouched: ["global_roles", "owner_global", "memberships", "organization_roles"] }, appliedAt: new Date(), updatedAt: new Date() }).where(eq(commercialEvents.id, row.id));
  await recordAuditLogBestEffort({ organizationId: profile.logtoOrganizationId, action: AUDIT_ACTIONS.COMMERCIAL_EVENT_APPLIED, result: AUDIT_RESULTS.SUCCESS, metadata: { eventId: normalized.eventId, eventType: normalized.eventType, seatTotal: next.seatTotal, consumedSeats, accessStatus: next.settings.commercial.accessStatus, logtoChange: "organization_custom_data_only_pending_access_enforcement_no_role_or_membership_mutation" } });
