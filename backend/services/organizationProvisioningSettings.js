...
function normalizeExtendedProvisioningInput(body = {}) {
  ...
  return { errors, value: { subdomain, slug, adminDomain, oidcRedirectUri, type: emptyToNull(body.organizationType ?? body.organization_type ?? body.crm?.type) || null, seatTotal: Number.isInteger(Number(body.seatTotal)) && Number(body.seatTotal) >= 0 ? Number(body.seatTotal) : 0 } };
}
