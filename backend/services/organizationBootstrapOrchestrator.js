...
const { ensureOrganizationTagsAndLists, getOrCreateCompanyForOrganization, normalizeCrmCompanyInput, upsertContactFromLogtoIdentity } = require("./fluentCrm");
...
function buildAuditContext({ authUser, internalUser, organization }) { return { actorLogtoUserId: authUser?.sub || null, actorInternalUserId: internalUser?.id || null, organizationId: organization?.id || organization?.organizationId || null, organizationName: getLogtoOrganizationName(organization) }; }
...
async function runDownstreamFluentCrm({ operation, canonical, extended, logtoOrganization, logtoOrganizationId, administrativeContactAssignments, internalUser, authUser, job }) {
  ...
  const normalizedCrm = normalizeCrmCompanyInput({ companyOwner: canonical.baseAdmin?.name, ...(operation.payloadSnapshotJson?.form?.crm || operation.payloadSnapshotJson?.form?.fluentcrm || {}) }, { name: canonical.name, nameCache: profile.nameCache, adminDomain: profile.adminDomain });
  ...
  const companyResult = await getOrCreateCompanyForOrganization(profile, { ...normalizedCrm, name: canonical.name, slug: extended.slug, adminDomain: extended.adminDomain }, { actorUserId: internalUser.id, auditMetadata: buildAuditContext({ authUser, internalUser, organization: logtoOrganization }) });
  ...
  for (const assignment of administrativeContactAssignments || []) {
    try { administrativeContacts.push({ ...assignment, contactSync: await upsertContactFromLogtoIdentity({ identity: { logtoUserId: assignment.logtoUserId, email: assignment.email, name: assignment.name, phone: assignment.phone, position: assignment.position }, companyId, roleNames: [assignment.roleName || assignment.organizationRoleName].filter(Boolean), extraLists: organizationLists }) }); }
...
async function processOrganizationBootstrapJob(job) {
  ...
  try {
    ...
    const canonicalResult = await runCanonicalOrganizationBootstrap({ canonical, logtoCustomData: buildLogtoOrganizationCustomData(extended), internalUser, auditContextBuilder: ({ organization }) => buildAuditContext({ authUser, internalUser, organization }) });
    ...
    const downstream = await runDownstreamFluentCrm({ operation, canonical, extended, logtoOrganization: canonicalResult.logtoOrganization, logtoOrganizationId: canonicalResult.logtoOrganizationId, administrativeContactAssignments: canonicalResult.administrativeContactAssignments, internalUser, authUser, job });
