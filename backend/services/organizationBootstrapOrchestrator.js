...
async function processOrganizationBootstrapJob(job) {
  ...
  try {
    ...
    const canonicalResult = await runCanonicalOrganizationBootstrap({ canonical, logtoCustomData: buildLogtoOrganizationCustomData(extended), internalUser, auditContextBuilder: ({ organization }) => buildAuditContext({ authUser, internalUser, organization, operationId, correlationId: operation.correlationId }) });
