...
const { processOrganizationBootstrapFoundationJob } = require("./workers/organizationBootstrapFoundation");
...
function createWorkers(connection = createRedisConnection()) {
  ...
  const worker = new Worker(QUEUE_NAMES.ORGANIZATION_BOOTSTRAP, processOrganizationBootstrapFoundationJob, { connection, prefix: getBullMqPrefix(), concurrency: Number.isInteger(concurrency) && concurrency > 0 ? concurrency : 2 });
