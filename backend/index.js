  ...
  try {
    ...
    if (canonicalCreated && logtoOrganizationId) {
      return res.status(201).json({
        ...
        failedStep: error.code === "LOGTO_ORGANIZATION_TEMPLATE_MISSING_ROLES" ? "organization_template_validation" : error.request?.path?.includes("/jit/") ? "jit_provisioning_configuration" : "base_admin_assignment",
