...
import type { ConsolidatedOperationalResponse, OperationalAction, OperationalBlock } from "../contracts/operational";
...
const actionLabel: Record<string, string> = {
  ...
  verify_provider: "Verificar proveedor en vivo",
  open_organization: "Abrir organización",
  wait_first_wordpress_login: "Esperar primer login WordPress",
  manual_retry_required: "Reintento manual requerido",
  human_action_required: "Acción humana requerida",
  none: "Sin acción requerida",
};

const severityVariant = (severity?: string | null) => severity === "success" ? "success" : severity === "critical" ? "danger" : severity === "warning" ? "warning" : "info";
const sourceLabel = (source?: string | null) => source === "live_provider_check" ? "Live provider check" : source === "worker_runtime" ? "Worker runtime" : source === "local_reconciled" ? "Local reconciled" : source === "persisted_snapshot" ? "Persisted snapshot" : source || "Sin fuente";
const formatDateTime = (value?: string | null) => value ? new Date(value).toLocaleString() : "No disponible";
const hasContactsBlocked = (state: ConsolidatedOperationalResponse) => [state.contactProgress, state.fluentcrm, state.liveVerification].some((block) => [block.status, block.providerCode, block.providerStatus, block.humanMessage, JSON.stringify(block.details ?? {})].some((value) => String(value ?? "").includes("contacts_not_started_due_to_company_failure") || String(value ?? "").includes("contacts_not_started")));

function OperationalBlockCard({ title, block, emphasis }: { title: string; block: OperationalBlock; emphasis?: boolean }) {
  return (
    <Card className={`h-100 shadow-sm ${emphasis ? "border-primary" : ""}`} data-testid={`operational-block-${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}>
      <Card.Body>
        <div className="d-flex justify-content-between align-items-start gap-2 mb-2">
          <div>
            <p className="text-uppercase text-secondary small fw-bold mb-1">{title}</p>
            <h3 className="h6 mb-0">{block.humanMessage || block.status}</h3>
          </div>
          <Badge bg={severityVariant(block.severity)}>{block.severity}</Badge>
        </div>
        <div className="d-flex flex-wrap gap-2 mb-3">
          <Badge bg={statusVariant(block.status)}>{block.status}</Badge>
          {block.freshness?.isStale ? <Badge bg="warning" text="dark">stale</Badge> : <Badge bg="secondary">fresh</Badge>}
          <Badge bg={block.freshness?.source === "live_provider_check" ? "primary" : block.freshness?.source === "worker_runtime" ? "dark" : "secondary"}>{sourceLabel(block.freshness?.source)}</Badge>
        </div>
        <dl className="row small mb-0">
          <dt className="col-5 text-secondary">Provider code</dt><dd className="col-7 text-break">{formatValue(block.providerCode)}</dd>
          <dt className="col-5 text-secondary">Provider status</dt><dd className="col-7 text-break">{formatValue(block.providerStatus)}</dd>
          <dt className="col-5 text-secondary">Checked at</dt><dd className="col-7">{formatDateTime(block.freshness?.checkedAt)}</dd>
          <dt className="col-5 text-secondary">Next action</dt><dd className="col-7 fw-semibold">{actionLabel[String(block.nextAction)] ?? block.nextAction}</dd>
          {block.invalidation?.lastEventId ? <><dt className="col-5 text-secondary">Invalidation</dt><dd className="col-7 text-break">{block.invalidation.lastEventId}</dd></> : null}
        </dl>
      </Card.Body>
    </Card>
  );
...
function OperationalActionPanel({ state, onRefresh }: { state: ConsolidatedOperationalResponse; onRefresh: () => void }) {
  const actions = state.summary.availableActions?.filter((action) => action !== state.summary.nextAction) ?? [];
  return (
    <PageCard title="Next action operacional" subtitle="Derivado únicamente del contrato operational-state; events y pending-sync quedan como detalle.">
      <div className="d-flex flex-column flex-lg-row justify-content-between gap-3">
        <div>
          <Badge bg={severityVariant(state.summary.severity)} className="mb-2">{state.summary.status}</Badge>
          <h3 className="h5">{state.summary.humanMessage}</h3>
...
function OperationalStatusCard({ data, state, onRefresh, isPolling }: { data: OwnerOrganizationProfileResponse; state?: ConsolidatedOperationalResponse; onRefresh: () => void; isPolling: boolean }) {
  if (!state) return <LoadingState title="Cargando operational-state" description="Leyendo contrato operacional consolidado." />;
  const liveDominates = state.liveVerification.freshness?.source === "live_provider_check";
  return (
    <div className="d-flex flex-column gap-4">
      <OperationalActionPanel state={state} onRefresh={onRefresh} />
      {isPolling ? <Alert variant="info">Polling activo por política backend: {state.polling.reason} ({state.polling.intervalSeconds}s). Operaciones: {state.polling.activeOperationIds.join(", ") || "no listadas"}.</Alert> : <Alert variant="success">Estado terminal o sin operación activa: polling detenido por el contrato backend.</Alert>}
      {liveDominates ? <Alert variant="primary">Existe <strong>live_provider_check</strong>; domina visualmente sobre reconciliación local y snapshots persistidos.</Alert> : null}
      {hasContactsBlocked(state) ? <Alert variant="warning"><strong>contacts_not_started_due_to_company_failure:</strong> los contactos no iniciaron porque FluentCRM Company falló o falta. Resolver Company desbloquea downstream de contactos.</Alert> : null}
      <div className="row g-3">
        <div className="col-12 col-xl-6"><OperationalBlockCard title="Logto canónico" block={state.canonical} /></div>
        <div className="col-12 col-xl-6"><OperationalBlockCard title="FluentCRM downstream" block={state.fluentcrm} /></div>
        <div className="col-12 col-xl-6"><OperationalBlockCard title="WordPress esperado" block={state.wordpress} /></div>
        <div className="col-12 col-xl-6"><OperationalBlockCard title="Worker y cola" block={state.worker} /></div>
        <div className="col-12 col-xl-6"><OperationalBlockCard title="Verificación live" block={state.liveVerification} emphasis={liveDominates} /></div>
        <div className="col-12 col-xl-6"><OperationalBlockCard title="Contactos downstream" block={state.contactProgress} /></div>
      </div>
      <PageCard title="Detalle secundario legacy" subtitle="pending-sync y events se mantienen para auditoría; no alimentan badges, CTA ni mensaje principal.">
        <dl className="row small mb-0">
          <dt className="col-sm-3 text-secondary">Pending sync</dt><dd className="col-sm-9">{data.sync.pending.length} elementos</dd>
          <dt className="col-sm-3 text-secondary">Events</dt><dd className="col-sm-9">{data.sync.events.length} eventos</dd>
          <dt className="col-sm-3 text-secondary">Ver logs</dt><dd className="col-sm-9"><Link to={`/owner/logs?organizationId=${encodeURIComponent(state.organization.sourceAnchors.logtoOrganizationId || "")}`}>Abrir timeline operativo</Link></dd>
        </dl>
      </PageCard>
    </div>
  );
...
function ProfileTab({ data, organizationId, onSaved }: { data: OwnerOrganizationProfileResponse; organizationId: string; onSaved: () => void }) {
  ...
  const operational = useOperationalState(organizationId, ownerApi.getOrganizationOperationalState);
