...
type TabKey = "profile" | "branding" | "logs" | "members";
...
function ProfileTab({ data, organizationId, onSaved }: { data: OwnerOrganizationProfileResponse; organizationId: string; onSaved: () => void }) {
  ...
  const primaryPending = data.sync.pending[0] ?? null;
...
function LogsTab({ data, onRetry, organizationId }: { data: OwnerOrganizationProfileResponse; onRetry: () => void; organizationId: string }) {
  const ownerApi = useOwnerApi();
  const { canExecute } = useAuthorization();
  const canRetrySync = canExecute("owner.organization.sync.retry");
  const manualResolve = (operationId: string) => ownerApi.resolveSyncOperationManually(organizationId, operationId, { resolutionType: "reviewed_no_action", resolutionReason: "Owner revisó el pendiente desde el centro operativo", notes: "No modifica FluentCRM/WordPress; cierre operativo local auditado." }).then(onRetry);
  return <div className="d-flex flex-column gap-4"><PageCard title="Pendientes y conflictos" subtitle="Pendientes downstream y estado técnico de cola por organización. La ventana del backend siempre incluye pendientes no terminales aunque sean antiguos.">{data.sync.pending.length === 0 ? <EmptyState title="Sin pendientes" description="El perfil no tiene reintentos pendientes." /> : <div className="row g-3">{data.sync.pending.map((item) => <div className="col-12 col-lg-6" key={item.id}><Alert variant={item.retryable ? "warning" : "secondary"} className="h-100"><div className="d-flex justify-content-between gap-2"><strong>{item.type}</strong><Badge bg="light" text="dark">{item.affectedSystem}</Badge></div><p className="mb-2 small">{cleanMessage(item.humanMessage || item.lastError)}</p>{item.missingFields?.length ? <div className="small text-warning mb-2">Campos faltantes: {item.missingFields.join(", ")}</div> : null}{item.fieldDiffs && Object.keys(item.fieldDiffs).length ? <div className="small text-info mb-2">Diferencias: {Object.keys(item.fieldDiffs).join(", ")}</div> : null}<dl className="small mb-3"><dt>Step</dt><dd>{item.stepName ?? "—"}</dd><dt>Cola / Job</dt><dd className="text-break">{item.queueName ?? "—"} · {item.jobId ?? "—"}</dd><dt>Retry</dt><dd>{item.retryState ?? item.status}{item.jobAgeSeconds != null ? ` · esperando ${formatDuration(item.jobAgeSeconds)}` : ""}</dd><dt>Worker</dt><dd>{item.workerHeartbeatState === "stale" ? "Job en cola, pero el worker no reporta actividad reciente" : item.retryState === "running" ? "Procesándose" : item.workerHeartbeatState ?? "unknown"}</dd>{item.suggestedSection ? <><dt>Remediación</dt><dd>{item.suggestedAction}</dd></> : null}</dl><div className="d-flex flex-wrap gap-2"><Button size="sm" variant="outline-primary" onClick={() => ownerApi.retrySyncOperation(organizationId, item.operationId).then(onRetry)} disabled={!item.retryable || !canRetrySync}>{canRetrySync ? "Reintentar" : "Solo lectura"}</Button>{item.canResendPayload ? <Button size="sm" variant="outline-success" disabled={!canRetrySync} onClick={() => ownerApi.resendSyncOperationPayload(organizationId, item.operationId).then(onRetry)}>Reenviar payload</Button> : null}{item.canVerifyProvider ? <Button size="sm" variant="outline-info" onClick={() => ownerApi.verifyOrganizationProvider(organizationId).then(onRetry)}>Verificar en proveedor</Button> : null}{item.canManualResolve ? <Button size="sm" variant="outline-secondary" disabled={!canRetrySync} onClick={() => manualResolve(item.operationId)}>Resolver manualmente</Button> : null}{item.suggestedRoute ? <Link className="btn btn-sm btn-outline-dark" to={item.suggestedRoute}>{item.ctaLabel || "Corregir datos"}</Link> : null}</div></Alert></div>)}</div>}</PageCard><PageCard title="Notificaciones" subtitle="Secuencia de eventos funcionales y auditoría enriquecida por organización.">{data.sync.events.length === 0 ? <EmptyState title="Sin eventos" description="No hay eventos operativos recientes." /> : <Table responsive hover><thead><tr><th>Fecha</th><th>Tipo</th><th>Resultado</th><th>Etapa</th><th>Mensaje</th><th>Cola / Job</th><th>Acción</th></tr></thead><tbody>{data.sync.events.map((event) => <tr key={event.id}><td>{event.at ? new Date(event.at).toLocaleString() : "—"}</td><td>{event.type}</td><td><Badge bg={event.result === "success" || event.result === "completed" ? "success" : event.requiresAction ? "warning" : "secondary"}>{event.result}</Badge></td><td>{event.stepName ?? event.stage}</td><td>{cleanMessage(event.humanMessage || event.message)}{event.providerCode ? <div className="small text-secondary">{event.providerCode}</div> : null}{event.retryState ? <div className="small text-secondary">Retry: {event.retryState}</div> : null}</td><td className="text-break small">{event.queueName ?? "—"}<br />{event.jobId ?? ""}</td><td>{event.retryOperationId ? <Button size="sm" variant="outline-primary" disabled={!canRetrySync} onClick={() => ownerApi.retrySyncOperation(organizationId, event.retryOperationId!).then(onRetry)}>{canRetrySync ? "Reintentar" : "Solo lectura"}</Button> : "—"}</td></tr>)}</tbody></Table>}</PageCard></div>;
}
...
export function OwnerOrganizationConsolePage() { const { organizationId = "" } = useParams(); const [searchParams] = useSearchParams(); const initialTab = (searchParams.get("tab") as TabKey) || "profile"; const [tab, setTab] = useState<TabKey>(initialTab); const ownerApi = useOwnerApi(); const resource = useStableResource({ initialParams: organizationId, load: ownerApi.getOrganizationProfile, getKey: (id) => `org-console-${id}` }); useEffect(() => { resource.retry(); }, [organizationId]); return <PageShell eyebrow="Consola por organización" title={resource.data?.organization.name ?? "Organización"} description="Perfil de tenant con datos canónicos de Logto, pendientes operativos y miembros."><Nav variant="tabs" activeKey={tab} onSelect={(key) => setTab((key as TabKey) || "profile")} className="mb-4"><Nav.Item><Nav.Link eventKey="profile">Datos de la organización</Nav.Link></Nav.Item><Nav.Item><Nav.Link eventKey="branding">Branding</Nav.Link></Nav.Item><Nav.Item><Nav.Link eventKey="logs">Logs operativos</Nav.Link></Nav.Item><Nav.Item><Nav.Link eventKey="members">Miembros</Nav.Link></Nav.Item></Nav>{resource.isLoading ? <LoadingState title="Cargando organización" description="Leyendo Logto, sync_operations y auditoría." /> : resource.error ? <ErrorState title="No se pudo cargar la consola" message={cleanMessage(resource.error)} action={<Button onClick={resource.retry}>Reintentar</Button>} /> : resource.data && tab === "profile" ? <ProfileTab data={resource.data} organizationId={organizationId} onSaved={resource.retry} /> : resource.data && tab === "branding" ? <BrandingTab data={resource.data} organizationId={organizationId} onSaved={resource.retry} /> : resource.data && tab === "logs" ? <LogsTab data={resource.data} organizationId={organizationId} onRetry={resource.retry} /> : <MembersTab organizationId={organizationId} />}</PageShell>; }
