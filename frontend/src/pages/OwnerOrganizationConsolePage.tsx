...
const formatSyncList = (values?: string[] | null) => values && values.length > 0 ? values.join(", ") : "—";
const formatDiffFields = (diffs?: Record<string, unknown> | null) => diffs && Object.keys(diffs).length > 0 ? Object.keys(diffs).join(", ") : "—";

function PendingSyncCard({ item, canRetrySync, onRetry }: { item: OwnerOrganizationProfileResponse["sync"]["pending"][number]; canRetrySync: boolean; onRetry: () => void }) {
  const message = item.humanMessage || item.lastError;
  const target = item.targetIdentity ? Object.values(item.targetIdentity).filter((value) => typeof value === "string" && value).join(" · ") : null;
  return (
    <Alert variant={item.retryable ? "warning" : item.providerStatus === "no_changes" ? "success" : "secondary"} className="h-100">
      <div className="d-flex justify-content-between gap-2">
        <div>
          <strong>{item.entityType ? `${item.entityType}: ${item.type}` : item.type}</strong>
          {target ? <div className="small text-secondary text-break">{target}</div> : null}
        </div>
        <Badge bg="light" text="dark">{item.affectedSystem}</Badge>
      </div>
      <p className="mb-2 small">{cleanMessage(message)}</p>
      <dl className="row small mb-3">
        <dt className="col-5">Step</dt><dd className="col-7 mb-1">{item.stepName || item.operationType || "—"}</dd>
        <dt className="col-5">Retry</dt><dd className="col-7 mb-1">{item.retryState || item.status}</dd>
        <dt className="col-5">Campos enviados</dt><dd className="col-7 mb-1">{formatSyncList(item.fieldsSent)}</dd>
        <dt className="col-5">Campos faltantes</dt><dd className="col-7 mb-1">{formatSyncList(item.missingFields)}</dd>
        <dt className="col-5">Cambios pendientes</dt><dd className="col-7 mb-1">{formatDiffFields(item.fieldDiffs)}</dd>
        <dt className="col-5">Proveedor</dt><dd className="col-7 mb-1">{[item.providerStatus, item.providerCode].filter(Boolean).join(" · ") || item.status}</dd>
        {item.queueName || item.jobId ? <><dt className="col-5 text-secondary">Cola/job</dt><dd className="col-7 mb-1 text-secondary">{[item.queueName, item.jobId].filter(Boolean).join(" · ")}</dd></> : null}
      </dl>
      {item.fieldDiffs && Object.keys(item.fieldDiffs).length > 0 ? <details className="small mb-3"><summary>Ver diferencias</summary><pre className="bg-light border rounded p-2 mt-2 mb-0 text-break">{JSON.stringify(item.fieldDiffs, null, 2)}</pre></details> : null}
      <Button size="sm" variant="outline-primary" onClick={onRetry} disabled={!item.retryable || !canRetrySync}>{canRetrySync ? item.suggestedAction : "Solo lectura"}</Button>
    </Alert>
  );
}
