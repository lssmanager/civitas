    ...
    <div className="px-4 py-4 border-bottom">
      <div className="d-flex align-items-center gap-2">
        ...
        <div>
          ...
          <p className="small text-secondary mb-0">Fase 01 · UI base</p>
        </div>
      </div>
    </div>
  );
...
export function SidebarNav({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <Nav className="flex-column gap-1 p-3" as="nav">
      {primaryNavigation.map((item) => (
        <NavLink
          ...
          onClick={onNavigate}
          className={({ isActive }) =>
            `nav-link civitas-sidebar-link rounded-3 px-3 py-2 ${isActive ? "active" : ""}`
          }
        >
